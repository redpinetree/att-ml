        ...
        if(lr==0){
            ...
            for(size_t i=0;i<new_w.nx();i++){
                for(size_t j=0;j<new_w.ny();j++){
                    new_w.at(i,j,current.f().at(i,j))=exp(old_current.w().at(i,j,0));
                    sum+=new_w.at(i,j,current.f().at(i,j));
                }
            }
            for(size_t i=0;i<new_w.nx();i++){
                for(size_t j=0;j<new_w.ny();j++){
                    ...
                    for(size_t i=0;i<new_w.nx();i++){
                        for(size_t j=0;j<new_w.ny();j++){
                            if((i<old_cluster[n].w().nx())&&(j<old_cluster[n].w().ny())){
                                new_w.at(i,j,0)=exp(old_cluster[n].w().at(i,j,0))*(old_cluster[n].w().nx()*old_cluster[n].w().ny())/(double) (new_w.nx()*new_w.ny());
                            }
                            else{
            ...
            for(size_t i=0;i<new_w.nx();i++){
                for(size_t j=0;j<new_w.ny();j++){
                    new_w.at(i,j,current.f().at(i,j))=unif_dist(mpi_utils::prng);
                    sum+=new_w.at(i,j,trial_current.f().at(i,j));
                }
            }
            for(size_t i=0;i<new_w.nx();i++){
                for(size_t j=0;j<new_w.ny();j++){
                    new_w.at(i,j,current.f().at(i,j))/=sum;
                    new_w.at(i,j,current.f().at(i,j))=log(new_w.at(i,j,current.f().at(i,j)));
                }
            }
            trial_current.w()=new_w;
            for(size_t n=0;n<trial_cluster.size();n++){
        ...
        for(size_t t=0;t<max_it;t++){
            ...
            for(size_t i=0;i<p_ij.nx();i++){
                for(size_t j=0;j<p_ij.ny();j++){
                    ...
                    sum_p_prime_ij_env_addends.push_back(p_prime_ij_env.at(i,j)+trial_current.w().at(i,j,current.f().at(i,j))); //restore divided factor to get z'
                }
            }
            ...
            cost=kl_div(exp(z),exp(z_prime),gi,gj,old_current,old_cluster,trial_current,trial_cluster,current.f());
            ...
            for(size_t i=0;i<p_ij.nx();i++){
                for(size_t j=0;j<p_ij.ny();j++){
                    if(lr==0){ //lr==0 means use iterative method based on stationarity condition
                        trial_current.w().at(i,j,current.f().at(i,j))=p_ij.at(i,j)-((fabs(p_prime_ij_env.at(i,j))<1e-10)?0:p_prime_ij_env.at(i,j));
                        sum_addends.push_back(trial_current.w().at(i,j,current.f().at(i,j)));
                    }
                    else{ //lr!=0 means use gradient descent with lr
                        double a=p_prime_ij_env.at(i,j)+trial_current.w().at(i,j,current.f().at(i,j));
                        ...
                        if(a!=b){ //a==b means grad C is 0, so no change to params
                            ...
                            x_current.at(i,j)=(1/(1+(alpha*0.01)))*(trial_current.w().at(i,j,current.f().at(i,j))-(alpha*u)); //win-adamw
                            trial_current.w().at(i,j,current.f().at(i,j))=(reckless_alpha*tau*x_current.at(i,j))+((alpha*tau)*(trial_current.w().at(i,j,current.f().at(i,j))-(reckless_alpha*u))); //win-adamw
                        }
                        sum_addends.push_back(trial_current.w().at(i,j,current.f().at(i,j)));
                    }
                }
            ...
            for(size_t i=0;i<trial_current.w().nx();i++){
                for(size_t j=0;j<trial_current.w().ny();j++){
                    trial_current.w().at(i,j,current.f().at(i,j))-=sum;
                    if(trial_current.w().at(i,j,current.f().at(i,j))<log(1e-100)){ //in case the weight is negative, force it to be nonnegative!
                        trial_current.w().at(i,j,current.f().at(i,j))=log(1e-100);
                    }
                }
            }
                ...
                for(size_t i=0;i<current.w().nx();i++){
                    for(size_t j=0;j<current.w().ny();j++){
                        ...
                        for(size_t imu=0;imu<p_ki.nx();imu++){
                            ...
                            factors_env.push_back(trial_current.w().at(i,j,current.f().at(i,j)));
                            factors_env.push_back(gi_prime[k]);
                            factors_env.push_back(gj_prime[k]);
                ...
                for(size_t imu=0;imu<p_ki.nx();imu++){
                    for(size_t k=0;k<p_ki.ny();k++){
                        if(lr==0){ //lr==0 means use iterative method based on stationarity condition
                            ...
                            double a=p_prime_ki_env.at(imu,k)+trial_cluster[n].w().at(imu,k,0);
                            ...
                            if(a!=b){ //a==b means grad C is 0, so no change to params
                                ...
                                double u=bias_corrected_m/(sqrt(bias_corrected_v)+epsilon); //win-adamw
                                x_cluster[n].at(imu,k)=(1/(1+(alpha*0.01)))*(trial_cluster[n].w().at(imu,k,0)-(alpha*u)); //win-adamw
                                trial_cluster[n].w().at(imu,k,0)=(reckless_alpha*tau*x_cluster[n].at(imu,k))+((alpha*tau)*(trial_cluster[n].w().at(imu,k,0)-(reckless_alpha*u))); //win-adamw
                            }
                            sum_addends.push_back(trial_cluster[n].w().at(imu,k,0));
                        }
                    }
...
array2d<size_t> optimize::f_maxent(site v_i,site v_j,array3d<double> w,size_t r_k){
    array2d<size_t> f_res(v_i.rank(),v_j.rank());
...
array2d<size_t> optimize::f_mvec_sim(site v_i,site v_j,array3d<double> w,size_t r_k){
    size_t r_i=v_i.rank();
    size_t r_j=v_j.rank();
    ...
    for(size_t k=0;k<r_k;k++){
        ...
        w_sums[k]+=exp_w.at(max_c_idx_i,max_c_idx_j);
...
array2d<size_t> optimize::f_hybrid_maxent(site v_i,site v_j,array3d<double> w,size_t r_k){
...
array2d<size_t> optimize::f_hybrid_mvec_sim(site v_i,site v_j,array3d<double> w,size_t r_k){
    if((v_i.rank()==v_j.rank())&&(v_i.rank()==r_k)){
        return f_alg1(v_i,v_j);
    }
    else{
...
double optimize::kl_div(double z,double z_prime,std::vector<double> gi,std::vector<double> gj,bond old_current,std::vector<bond> old_cluster,bond current,std::vector<bond> cluster,array2d<size_t> f){
    ...
    for(size_t i=0;i<old_current.w().nx();i++){
        for(size_t j=0;j<old_current.w().ny();j++){
            double contrib_a=exp(old_current.w().at(i,j,0))*exp(gi[i])*exp(gj[j])*old_current.w().at(i,j,0);
            double contrib_b=exp(old_current.w().at(i,j,0))*((gi_tilde.at(i,j)*exp(gj[j]))+(exp(gi[i])*gj_tilde.at(i,j)));
            a+=contrib_a;
            b+=contrib_b;
        }
    }
    ...
    for(size_t i=0;i<current.w().nx();i++){
        for(size_t j=0;j<current.w().ny();j++){
            double contrib_a_prime=exp(old_current.w().at(i,j,0))*exp(gi[i])*exp(gj[j])*current.w().at(i,j,f.at(i,j));
            double contrib_b_prime=exp(old_current.w().at(i,j,0))*((gi_prime_tilde.at(i,j)*exp(gj[j]))+(exp(gi[i])*gj_prime_tilde.at(i,j)));
