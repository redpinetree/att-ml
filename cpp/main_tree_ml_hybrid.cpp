...
void print_usage(){
    ...
    std::cerr<<"\t--compress-r: enable rank compression in TTNBM training\n";
    std::cerr<<"\t--struct-opt: enable structural optimization in TTNBM training\n";
    std::cerr<<"\t--compress-r2: enable rank compression in nnTTN training\n";
    std::cerr<<"\t--struct-opt2: enable structural optimization in nnTTN training\n";
}
...
int main(int argc,char **argv){
    ...
    while(1){
        static struct option long_opts[]={
            ...
            {"compress-r2",no_argument,&compress_r2,1},
            {"struct-opt2",no_argument,&struct_opt2,1},
...

    {
        ...
        std::cout<<"\tinit tree type: "<<"ttnbm"<<"\n";
        std::cout<<"\ttop dim: "<<tdim<<"\n";
        std::cout<<"\tr max: "<<r_max<<"\n";
        std::cout<<"\tnll iter max: "<<n_nll_iter_max<<"\n";
        std::cout<<"\tlearning rate: "<<lr<<"\n";
        std::cout<<"\tbatch size: "<<batch_size<<"\n";
        std::cout<<"\tstruct opt: "<<(struct_opt2?"true":"false")<<"\n";
        std::cout<<"\tcompress r: "<<(compress_r2?"true":"false")<<"\n";
        sw.start();
        std::map<size_t,double> train_nll_history,test_nll_history;
        std::map<size_t,size_t> sweep_history;
        ...
        if(label_set&&test_set&&test_label_set){
            algorithm::train_nll(g,train_data,train_data_labels,test_data,test_data_labels,n_nll_iter_max,r_max,compress_r2,lr,batch_size,train_nll_history,test_nll_history,sweep_history,struct_opt2); //nll training with labels and test data
        }
        else if(label_set&&(!test_set)){
            algorithm::train_nll(g,train_data,train_data_labels,n_nll_iter_max,r_max,compress_r2,lr,batch_size,train_nll_history,sweep_history,struct_opt2); //nll training with labels
        }
        else if((!label_set)&&test_set){
            algorithm::train_nll(g,train_data,test_data,n_nll_iter_max,r_max,compress_r2,lr,batch_size,train_nll_history,test_nll_history,sweep_history,struct_opt2); //nll training with test data
        }
        else{
            algorithm::train_nll(g,train_data,n_nll_iter_max,r_max,compress_r2,lr,batch_size,train_nll_history,sweep_history,struct_opt2); //nll training
        }
        sw.split();
        if(verbose>=3){std::cout<<"nll training time: "<<(double) sw.elapsed()<<"ms\n";}
        trial_time+=sw.elapsed();
