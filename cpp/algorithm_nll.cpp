...
std::vector<double> algorithm::train_nll(graph<cmp>& g,size_t n_cycles,size_t n_samples,size_t iter_max){
    ...
    std::vector<sample_data> samples=sampling::local_mh_sample(g,n_samples,0);
    std::cout<<n_samples<<" samples generated via local MH\n";
    sampling::mh_sample(g,n_samples,acceptance_ratio,0);
    // std::vector<sample_data> samples=sampling::mh_sample(g,n_samples,acceptance_ratio,0);
    ...
    for(size_t c=1;c<=n_cycles;c++){
        ...
        // double nll=optimize::hopt_nll2(g,n_samples,iter_max);
        for(auto it=g.es().begin();it!=g.es().end();++it){
            bond current=*it;
            algorithm::calculate_site_probs(g,current);
        }
        samples=sampling::local_mh_sample(g,n_samples,0);
        std::cout<<n_samples<<" samples generated via local MH\n";
        sampling::mh_sample(g,n_samples,acceptance_ratio,0);
        // samples=sampling::mh_sample(g,n_samples,acceptance_ratio,0);
        acceptance_ratios.push_back(acceptance_ratio);
