...
template<typename cmp>
void algorithm::train_nll(graph<cmp>& g,std::vector<sample_data>& samples,std::vector<size_t>& labels,size_t iter_max,size_t r_max,double lr,std::vector<double>& nll_history){
    // double nll=optimize::opt_nll(g,samples,labels,iter_max);
    double nll=optimize::opt_struct_nll(g,samples,labels,iter_max,r_max,lr,nll_history);
...
template void algorithm::train_nll(graph<bmi_comparator>&,std::vector<sample_data>&,std::vector<size_t>&,size_t,size_t,double,std::vector<double>&);

template<typename cmp>
void algorithm::train_nll(graph<cmp>& g,std::vector<sample_data>& samples,size_t iter_max,size_t r_max,double lr,std::vector<double>& nll_history){
    std::vector<size_t> dummy_labels;
    // double nll=optimize::opt_nll(g,samples,dummy_labels,iter_max);
    double nll=optimize::opt_struct_nll(g,samples,dummy_labels,iter_max,r_max,lr,nll_history);
    for(auto it=g.es().begin();it!=g.es().end();++it){
        bond current=*it;
        algorithm::calculate_site_probs(g,current);
    }
...
template void algorithm::train_nll(graph<bmi_comparator>&,std::vector<sample_data>&,size_t,size_t,double,std::vector<double>&);
