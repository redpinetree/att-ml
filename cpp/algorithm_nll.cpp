...
template<typename cmp>
void algorithm::train_nll(graph<cmp>& g,std::vector<sample_data>& samples,std::vector<size_t>& labels,size_t iter_max,size_t r_max,bool compress_r,double lr,std::map<size_t,double>& nll_history,std::map<size_t,size_t>& sweep_history,bool struct_opt){
    // double nll=optimize::opt_nll(g,samples,labels,iter_max);
    double nll=optimize::opt_struct_nll(g,samples,labels,iter_max,r_max,compress_r,lr,nll_history,sweep_history,struct_opt);
...
template<typename cmp>
void algorithm::train_nll(graph<cmp>& g,std::vector<sample_data>& samples,size_t iter_max,size_t r_max,bool compress_r,double lr,std::map<size_t,double>& nll_history,std::map<size_t,size_t>& sweep_history,bool struct_opt){
    std::vector<size_t> dummy_labels;
    // double nll=optimize::opt_nll(g,samples,dummy_labels,iter_max);
    double nll=optimize::opt_struct_nll(g,samples,dummy_labels,iter_max,r_max,compress_r,lr,nll_history,sweep_history,struct_opt);
    for(auto it=g.es().begin();it!=g.es().end();++it){
        bond current=*it;
        algorithm::calculate_site_probs(g,current);
    }
