...
double optimize::opt_struct_nll_born(graph<cmp>& g,std::vector<sample_data>& train_samples,std::vector<size_t>& train_labels,std::vector<sample_data>& test_samples,std::vector<size_t>& test_labels,size_t iter_max,size_t r_max,bool compress_r,double lr,std::map<size_t,double>& train_nll_history,std::map<size_t,double>& test_nll_history,std::map<size_t,size_t>& sweep_history,bool struct_opt){
    ...
    std::vector<double> w=calc_w_born(g,train_samples,train_labels,l_env_sample,r_env_sample,u_env_sample); //also calculate envs
    ...
    while(iter<iter_max){
        ...
        canonicalize(g,(*g.es().begin()).order());
        z=calc_z_born(g);
        w=calc_w_born(g,train_samples,train_labels,l_env_sample,r_env_sample,u_env_sample); //also calculate envs
        auto it=g.es().begin();
        ...
        // for(auto it=g.es().begin();it!=--g.es().end();++it){
            ...
            double bmi1=way1_born(g,it,it_parent,current,parent,z,w,l_env_sample,r_env_sample,u_env_sample,fused,r_max,compress_r,train_samples,train_labels,single_site_update_count,lr,beta1,beta2,epsilon);
            ...
            if(struct_opt){
                ...
                double bmi2=way2_born(g,it,it_parent,current,parent,z,w,l_env_sample,r_env_sample,u_env_sample,fused,r_max,compress_r,train_samples,train_labels,single_site_update_count,lr,beta1,beta2,epsilon);
                ...
                double bmi3=way3_born(g,it,it_parent,current,parent,z,w,l_env_sample,r_env_sample,u_env_sample,fused,r_max,compress_r,train_samples,train_labels,single_site_update_count,lr,beta1,beta2,epsilon);
    ...
    for(size_t i=0;i<g.n_phys_sites();i++){
        canonicalize(g,g.vs()[i].u_idx());
        w=calc_w_born(g,train_samples,train_labels,l_env_sample,r_env_sample,u_env_sample);
        // double input_bmi=fabs(optimize::calc_bmi_input(i,g.vs()[i].rank(),train_samples,g.vs()[g.vs()[i].u_idx()].p_bond(),w,l_env_sample,r_env_sample,u_env_sample)) //take abs
        double input_bmi=optimize::calc_bmi_input_born(g,i,g.vs()[i].rank(),g.vs()[g.vs()[i].u_idx()].p_bond(),w,l_env_sample,r_env_sample,u_env_sample);
        // std::cout<<input_bmi<<"\n";
...
template double optimize::opt_struct_nll_born(graph<bmi_comparator>&,std::vector<sample_data>&,std::vector<size_t>&,std::vector<sample_data>&,std::vector<size_t>&,size_t,size_t,bool,double,std::map<size_t,double>&,std::map<size_t,double>&,std::map<size_t,size_t>&,bool);
...
template<typename cmp>
double optimize::calc_bmi_input_born(graph<cmp>& g,size_t idx,size_t input_rank,bond& current,std::vector<double>& w,std::vector<std::vector<array1d<double> > >& l_env_sample,std::vector<std::vector<array1d<double> > >& r_env_sample,std::vector<std::vector<array1d<double> > >& u_env_sample){
    if(input_rank==1){return 0;} //bmi is nonnegative and upper-bounded by bond dimension, so dim=1 -> bmi=0
    //assumes that the ttn is canonicalized with center at current (the tensor above the input site), and p_bonds are accurate
    size_t n_samples=w.size();
    std::vector<double> a_subsystem_sample(n_samples);
    std::vector<double> b_subsystem_sample(n_samples);
    std::vector<double> ab_subsystem_sample(n_samples);
    ...
    for(size_t s=0;s<n_samples;s++){
        std::vector<double> a_subsystem_sample_addends;
        if(idx==current.v1()){
            for(size_t k1=0;k1<current.w().nx();k1++){
                for(size_t k2=0;k2<current.w().nx();k2++){
                    for(size_t l=0;l<current.w().ny();l++){
                        for(size_t m=0;m<current.w().nz();m++){
                            a_subsystem_sample_addends.push_back(l_env_sample[current.order()][s].at(k1)*l_env_sample[current.order()][s].at(k2)*current.w().at(k1,l,m)*current.w().at(k2,l,m));
                        }
                    }
                }
            }
            ...
            for(size_t k=0;k<current.w().nx();k++){
                for(size_t l1=0;l1<current.w().ny();l1++){
                    for(size_t l2=0;l2<current.w().ny();l2++){
                        for(size_t m=0;m<current.w().nz();m++){
                            a_subsystem_sample_addends.push_back(r_env_sample[current.order()][s].at(l1)*r_env_sample[current.order()][s].at(l2)*current.w().at(k,l1,m)*current.w().at(k,l2,m));
                        }
                    }
                }
            }
        ...
        if(idx==current.v1()){
            for(size_t k=0;k<b_subsystem_tensor_sample[s].nx();k++){
                for(size_t l1=0;l1<b_subsystem_tensor_sample[s].ny();l1++){
                    for(size_t l2=0;l2<b_subsystem_tensor_sample[s].ny();l2++){
                        for(size_t m=0;m<b_subsystem_tensor_sample[s].nz();m++){
                            b_subsystem_sample_addends.push_back(r_env_sample[current.order()][s].at(l1)*r_env_sample[current.order()][s].at(l2)*b_subsystem_tensor_sample[s].at(k,l1,m)*b_subsystem_tensor_sample[s].at(k,l2,m));
                        }
                    }
                }
            }
            ...
            for(size_t k1=0;k1<b_subsystem_tensor_sample[s].nx();k1++){
                for(size_t k2=0;k2<b_subsystem_tensor_sample[s].nx();k2++){
                    for(size_t l=0;l<b_subsystem_tensor_sample[s].ny();l++){
                        for(size_t m=0;m<b_subsystem_tensor_sample[s].nz();m++){
                            b_subsystem_sample_addends.push_back(l_env_sample[current.order()][s].at(k1)*l_env_sample[current.order()][s].at(k2)*b_subsystem_tensor_sample[s].at(k1,l,m)*b_subsystem_tensor_sample[s].at(k2,l,m));
                        }
                    }
                }
            }
    ...
    // std::cout<<"s_b: "<<s_b<<"\n";
    // std::cout<<"s_ab: "<<s_ab<<"\n";
    // std::cout<<"("<<idx<<","<<current.order()<<") bmi: "<<bmi<<"\n";
    return bmi;
}
template double optimize::calc_bmi_input_born(graph<bmi_comparator>&,size_t,size_t,bond&,std::vector<double>&,std::vector<std::vector<array1d<double> > >&,std::vector<std::vector<array1d<double> > >&,std::vector<std::vector<array1d<double> > >&);
