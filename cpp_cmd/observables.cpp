...
std::map<std::tuple<size_t,size_t,size_t,size_t,std::vector<size_t>,std::vector<double> >,std::complex<double> > observables::known_factors_complex;
...
double observables::m(graph<cmp>& g,size_t root,size_t q,size_t n,size_t p,std::vector<size_t> c,size_t depth){
    ...
    for(size_t k=0;k<q;k++){
        for(size_t j=0;j<q;j++){
            for(size_t i=0;i<q;i++){
                double e=(f(g.vs()[g.vs()[root].p1()],i,g.vs()[g.vs()[root].p2()],j)==k)?q*(*g.vs()[root].adj().begin()).w().at(i,j):0;
                prob_ij.at(k,j,i)=e;
...
//this m function is just a wrapper for the top-down version
template<typename cmp>
std::complex<double> observables::m(graph<cmp>& g,size_t root,size_t q,size_t n,size_t p,std::vector<double> k,size_t depth){
    std::vector<size_t> c(q,0);
    c[0]=p;
    return observables::m(g,root,q,n,p,c,k,depth);
}
template std::complex<double> observables::m<coupling_comparator>(graph<coupling_comparator>&,size_t,size_t,size_t,size_t,std::vector<double>,size_t);
template std::complex<double> observables::m<bmi_comparator>(graph<bmi_comparator>&,size_t,size_t,size_t,size_t,std::vector<double>,size_t);

//this m function calculates using a top-down approach. theoretically, this is sufficient to calculate observables, but due to stack size limitations, a segfault occurs with a stack overflow for large lattices.
template<typename cmp>
std::complex<double> observables::m(graph<cmp>& g,size_t root,size_t q,size_t n,size_t p,std::vector<size_t> c,std::vector<double> k,size_t depth){
    // std::cout<<depth<<"\n";
    size_t sum_c=0;
    for(size_t i=0;i<c.size();i++){
        sum_c+=c[i];
    ...
    for(size_t down=0;down<2;down++){ //every non-leaf site has 2 downstream sites
        ...
        for(size_t s=0;s<spin_combos.size();s++){
            ...
            try{
                contrib=observables::known_factors_complex.at(std::make_tuple((down==0)?g.vs()[root].p1():g.vs()[root].p2(),q,n,p,c_vals,k));
            }
            catch(const std::out_of_range& oor){
                contrib=m(g,(down==0)?g.vs()[root].p1():g.vs()[root].p2(),q,n,p,c_vals,k,depth+1);
                observables::known_factors_complex[std::make_tuple((down==0)?g.vs()[root].p1():g.vs()[root].p2(),q,n,p,c_vals,k)]=contrib;
            }
            double weight=1;
            for(size_t i=0;i<p;i++){
                weight*=(down==0)?prob_i.at(q_idxs[i],spin_combos[s][i]):prob_j.at(q_idxs[i],spin_combos[s][i]);
    ...
    for(size_t comp=1;comp<n;comp++){
        ...
        for(size_t s0=0;s0<spin_combos.size();s0++){
            for(size_t s1=0;s1<spin_combos.size();s1++){
                ...
                //memoize
                try{
                    factors[0]=known_factors_complex.at(std::make_tuple(g.vs()[root].p1(),q,c0,p,c_vals0,k));
                }
                catch(const std::out_of_range& oor){
                    factors[0]=m(g,g.vs()[root].p1(),q,c0,p,c_vals0,k,depth+1);
                    known_factors_complex[std::make_tuple(g.vs()[root].p1(),q,c0,p,c_vals0,k)]=factors[0];
                }
                try{
                    factors[1]=known_factors_complex.at(std::make_tuple(g.vs()[root].p2(),q,c1,p,c_vals1,k));
                }
                catch(const std::out_of_range& oor){
                    factors[1]=m(g,g.vs()[root].p2(),q,c1,p,c_vals1,k,depth+1);
                    known_factors_complex[std::make_tuple(g.vs()[root].p2(),q,c1,p,c_vals1,k)]=factors[1];
                }
...
template std::complex<double> observables::m<coupling_comparator>(graph<coupling_comparator>&,size_t,size_t,size_t,size_t,std::vector<size_t>,std::vector<double>,size_t);
template std::complex<double> observables::m<bmi_comparator>(graph<bmi_comparator>&,size_t,size_t,size_t,size_t,std::vector<size_t>,std::vector<double>,size_t);

//this m function is just a wrapper for the bottom-up version
template<typename cmp>
std::complex<double> observables::m(graph<cmp>& g,size_t q,size_t n,size_t p,std::vector<double> k){
    std::vector<size_t> c(q,0);
    c[0]=p;
    return observables::m(g,q,n,p,c,k);
}
...
//this m function calculates using a bottom-up approach. this fixes the stack size issue since all top-down calls resolve with depth 1. this function still requires the top-down version to compute observables as the tree is traversed towards the root.
template<typename cmp>
std::complex<double> observables::m(graph<cmp>& g,size_t q,size_t n,size_t p,std::vector<size_t> c,std::vector<double> k){
    size_t sum_c=0;
    for(size_t i=0;i<c.size();i++){
        sum_c+=c[i];
    }
    ...
    for(auto it=g.es().begin();it!=g.es().end();++it){
        std::array<size_t,2> v_idxs{(*it).v1(),(*it).v2()};
        known_factors_complex[std::make_tuple(v_idxs[0],q,n,p,c,k)]=m(g,v_idxs[0],q,n,p,c,k,0);
        known_factors_complex[std::make_tuple(v_idxs[1],q,n,p,c,k)]=m(g,v_idxs[1],q,n,p,c,k,0);
    }
    return m(g,g.vs().size()-1,q,n,p,c,k,0);
}
template std::complex<double> observables::m<coupling_comparator>(graph<coupling_comparator>&,size_t,size_t,size_t,std::vector<size_t>,std::vector<double>);
template std::complex<double> observables::m<bmi_comparator>(graph<bmi_comparator>&,size_t,size_t,size_t,std::vector<size_t>,std::vector<double>);
