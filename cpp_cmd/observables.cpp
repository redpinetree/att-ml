...
double observables::m(graph<cmp>& g,size_t root,size_t n_target,size_t p,bool abs_flag){
    ...
    // res*=pow(m_prefactor(n_target,r_k),n_target*p);
    return res;
}
...
double observables::q(graph<cmp>& g,size_t root,size_t n_target,size_t n,size_t p,std::vector<size_t> r,std::vector<size_t> c,size_t depth){
    ...
    for(size_t down=0;down<2;down++){ //every non-leaf site has 2 downstream sites
        ...
        for(size_t s=0;s<spin_combos[down].size();s++){
            ...
            // if(depth==0 && n_target==2 && p==2){std::cout<<root<<" "<<((down==0)?g.vs()[root].p1():g.vs()[root].p2())<<" "<<n<<" "<<p<<" "<<weight<<","<<contrib<<"\n";}
            temp+=weight*contrib;
        }
        // if(depth==0){std::cout<<"subtree "<<down<<":"<<temp<<"\n";}
        c_res+=temp;
    ...
    for(size_t comp=1;comp<n;comp++){
        ...
        for(size_t s0=0;s0<spin_combos[0].size();s0++){
            for(size_t s1=0;s1<spin_combos[1].size();s1++){
                ...
                // if(depth==0 && n_target==2 && p==2){std::cout<<weight<<","<<factors[0]<<","<<factors[1]<<","<<factor_prod<<"\n";}
                c_res+=coef*weight*factor_prod;
            }
        }
        // if(depth==0){std::cout<<"c_res ("<<c1<<","<<c0<<") "<<coef<<": "<<c_res<<"\n";}
...
template double observables::q<coupling_comparator>(graph<coupling_comparator>&,size_t,size_t,size_t,size_t,std::vector<size_t>,std::vector<size_t>,size_t);
...
template<typename cmp>
double observables::q(graph<cmp>& g,size_t root,size_t n_target,size_t p,bool abs_flag){
    // std::vector<size_t> c(g.vs()[root].rank(),0);
    // c[0]=p;
    ...
    for(size_t n=2;n<=n_target;n++){
        ...
        res+=pow(-1,(double) n)*binom(n_target,n_target-n)*pow(3,(double) n-n_target)*pow(g.n_phys_sites(),n_target-n)*n_res;
    }
    ...
    // std::cout<<pow(-1,(double) n_target-1)*(n_target-1)*pow(2,-(double) n_target)*pow(g.n_phys_sites(),n_target)<<"\n\n";
    res+=pow(-1,(double) n_target-1)*(n_target-1)*pow(3,-(double) n_target)*pow(g.n_phys_sites(),n_target);
    res*=pow(q_prefactor(n_target,3),p-1);
    // std::cout<<"final_res: "<<res<<"\n";
    return res;
}
template double observables::q<coupling_comparator>(graph<coupling_comparator>&,size_t,size_t,size_t,bool);
template double observables::q<bmi_comparator>(graph<bmi_comparator>&,size_t,size_t,size_t,bool);
...
template double observables::q<coupling_comparator>(graph<coupling_comparator>&,size_t,size_t,size_t,std::vector<size_t>,std::vector<size_t>);
...
std::complex<double> observables::m(graph<cmp>& g,size_t root,size_t n_target,size_t n,size_t p,std::vector<size_t> r,std::vector<size_t> c,std::vector<double> k,size_t depth){
    ...
    for(size_t down=0;down<2;down++){ //every non-leaf site has 2 downstream sites
        ...
        for(size_t s=0;s<spin_combos[down].size();s++){
            ...
            try{
                contrib=observables::m_known_factors_complex.at(std::make_tuple((down==0)?g.vs()[root].p1():g.vs()[root].p2(),n_target,n,p,r,c_vals,k));
            }
            catch(const std::out_of_range& oor){
                contrib=m(g,(down==0)?g.vs()[root].p1():g.vs()[root].p2(),n_target,n,p,r,c_vals,k,depth+1);
                observables::m_known_factors_complex[std::make_tuple((down==0)?g.vs()[root].p1():g.vs()[root].p2(),n_target,n,p,r,c_vals,k)]=contrib;
            }
            double weight=1;
            for(size_t i=0;i<p;i++){
                weight*=(down==0)?prob_i.at(spin_combos[down][s][i],c[i]):prob_j.at(spin_combos[down][s][i],c[i]);
    ...
    for(size_t comp=1;comp<n;comp++){
        ...
        for(size_t s0=0;s0<spin_combos[0].size();s0++){
            for(size_t s1=0;s1<spin_combos[1].size();s1++){
                ...
                //memoize
                try{
                    factors[0]=m_known_factors_complex.at(std::make_tuple(g.vs()[root].p1(),n_target,c0,p,r,c_vals0,k));
                }
                catch(const std::out_of_range& oor){
                    factors[0]=m(g,g.vs()[root].p1(),n_target,c0,p,r,c_vals0,k,depth+1);
                    m_known_factors_complex[std::make_tuple(g.vs()[root].p1(),n_target,c0,p,r,c_vals0,k)]=factors[0];
                }
                try{
                    factors[1]=m_known_factors_complex.at(std::make_tuple(g.vs()[root].p2(),n_target,c1,p,r,c_vals1,k));
                }
                catch(const std::out_of_range& oor){
                    factors[1]=m(g,g.vs()[root].p2(),n_target,c1,p,r,c_vals1,k,depth+1);
                    m_known_factors_complex[std::make_tuple(g.vs()[root].p2(),n_target,c1,p,r,c_vals1,k)]=factors[1];
                }
...
std::complex<double> observables::m(graph<cmp>& g,size_t n_target,size_t n,size_t p,std::vector<size_t> r,std::vector<size_t> c,std::vector<double> k){
    ...
    for(auto it=g.es().begin();it!=g.es().end();++it){
        ...
        std::vector<size_t> c0(p,0);
        std::vector<size_t> c1(p,0);
        m_known_factors_complex[std::make_tuple(v_idxs[0],n_target,n,p,r,c0,k)]=m(g,v_idxs[0],n_target,n,p,r,c0,k,0);
        m_known_factors_complex[std::make_tuple(v_idxs[1],n_target,n,p,r,c1,k)]=m(g,v_idxs[1],n_target,n,p,r,c1,k,0);
    }
...
template<typename cmp>
void observables::print_moments(graph<cmp>& g,size_t q){ //debug
...
template void observables::print_moments<coupling_comparator>(graph<coupling_comparator>&,size_t);
