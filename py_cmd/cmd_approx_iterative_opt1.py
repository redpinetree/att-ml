...
while beta<=args.max_beta:
	...
	# print(bonds)
	start=time.time()
	bonds.sort(key=lambda b: (b.up is None,b.bmi,2-(sites[b.v1].virtual_flag+sites[b.v2].virtual_flag),b.v1,b.v2)) #bonds[-1] is the heaviest (bmi) bond that is yet to be contracted
	# bonds.sort(key=lambda b: (b.up is None,np.abs(0.5*np.log(b.w[0,0]/b.w[0,1])),2-(sites[b.v1].virtual_flag+sites[b.v2].virtual_flag)))
	# print(bonds)
	...
	while bonds[-1].up is None:
		...
		v1_orig_idx=sites[bonds[-1].v1].original if sites[bonds[-1].v1].virtual_flag else bonds[-1].v1
		v2_vol=sites[bonds[-1].v2].vol
		v2_orig_idx=sites[bonds[-1].v2].original if sites[bonds[-1].v2].virtual_flag else bonds[-1].v2
		if v1_vol==v2_vol:
		...
		for iter in range(1000):
			w_ij_old=copy.deepcopy(bonds[-1].w)
			pair_ij=np.zeros(bonds[-1].w.shape) #pair distribution of considered bond
			...
			if np.all(bonds[-1].w-w_ij_old<1e-10):
		...
		for m in bonds_group1:
			for iter in range(1000):
				w_ki_old=copy.deepcopy(bonds[m].w)
				...
				for imu in range(pair_ki.shape[0]):
					for i in range(bonds[-1].w.shape[0]):
						for j in range(bonds[-1].w.shape[1]):
							...
							# print(g1,g2)
				pair_ki/=np.sum(pair_ki)
				pair_ki_env/=np.sum(bonds[m].w*pair_ki_env)
				bonds[m].w=pair_ki/pair_ki_env
				if np.all(bonds[m].w-w_ki_old<1e-10):
					print("converged after %d iterations"%(iter+1))
					break
			print(sites[bonds[-1].v1],sites[bonds[-1].v2],"w_ki:",bonds[m].w)
			print("")
		
		for n in bonds_group2:
			for iter in range(1000):
				w_kj_old=copy.deepcopy(bonds[n].w)
				...
				for jnu in range(pair_kj.shape[0]):
					for i in range(bonds[-1].w.shape[0]):
						for j in range(bonds[-1].w.shape[1]):
							...
							# print(j,[np.sum(bonds[b].w,axis=0 if bonds[b].v1==bonds[-1].v2 else 1)[j] for b in bonds_group2])
							# print(g1,g2)
				pair_kj/=np.sum(pair_kj)
				pair_kj_env/=np.sum(bonds[n].w*pair_kj_env)
				bonds[n].w=pair_kj/pair_kj_env
				if np.all(bonds[n].w-w_kj_old<1e-10):
					print("converged after %d iterations"%(iter+1))
					break
			print(sites[bonds[-1].v1],sites[bonds[-1].v2],"w_kj:",bonds[n].w)
			print("")
		...
		bonds.sort(key=lambda b: (b.up is None,b.bmi,2-(sites[b.v1].virtual_flag+sites[b.v2].virtual_flag),b.v1,b.v2))
		# bonds.sort(key=lambda b: (b.up is None,np.abs(0.5*np.log(b.w[0,0]/b.w[0,1])),2-(sites[b.v1].virtual_flag+sites[b.v2].virtual_flag)))
		# sys.exit(1)
